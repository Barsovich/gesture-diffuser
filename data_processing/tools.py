...
def create_bvh(filename, prediction, frame_time):
    ...
    with open(filename, 'w') as fo:
        ...
        offset = [0, 60, 0]
        offset_line = "\tOFFSET " + \
            " ".join("{:.6f}".format(x) for x in offset) + '\n'
        fo.write("HIERARCHY\n")
        fo.write("ROOT Hips\n")
        fo.write("{\n")
...
def calculate_mfcc(audio_filename):
    ...
    input_vectors = mfcc(audio, winlen=0.02, winstep=0.00833,
                         samplerate=fs, numcep=MFCC_INPUTS, nfft=NFFT)

    input_vectors = [average(input_vectors[:, i], 4)
...
def extract_prosodic_features(audio_filename):
    ...
    min_size = min(len(energy), len(energy_der),
                   len(pitch_der), len(pitch_der))
