...
class BVHParser():
    ...
    def parse(self, filename, start=0, stop=-1):
        ...
        print("self.data.skeleton", self.data.skeleton)
        print("self.data.channel_names", self.data.channel_names)
        print("self.data.values", self.data.values)
        print("self.data.root_name", self.data.root_name)
        print("self.data.framerate", self.data.framerate)
    ...
    def _to_DataFrame(self):
        ...
        channels = np.asarray([[channel[2] for channel in frame]
                              for frame in frames])
    ...
    def _new_bone(self, parent, name):
        bone = {'parent': parent, 'channels': [],
                'offsets': [], 'order': '', 'children': []}
        return bone
